...
#pragma mark - Key Code and Glyph Mappings
...
static const macOS_glyph_desc_t key_glyphs[0xFF] = {
...
{
    if (virtual_keys[code].is_valid) {
        return virtual_keys[code].code;
    } else {
        return INVALID_KEY;
    }
}
...
obs_key_t obs_key_from_virtual_key(int code)
{
    ...
    for (size_t i = 0; i < OBS_KEY_LAST_VALUE; i++) {
        if (virtual_keys[i].is_valid && virtual_keys[i].code == code) {
            return (obs_key_t) i;
        }
    }
    return OBS_KEY_NONE;
...
static bool localized_key_to_str(obs_key_t key, struct dstr *str)
{
    if (key < OBS_KEY_LAST_VALUE) {
        if (key_descriptions[key].is_valid) {
...
{
    if (code < INVALID_KEY) {
        if (key_glyphs[code].is_valid && key_glyphs[code].is_glyph && key_glyphs[code].glyph > 0) {
            dstr_from_wcs(str, (wchar_t[]) {key_glyphs[code].glyph, 0});
        } else if (key_glyphs[code].is_valid && key_glyphs[code].desc) {
            dstr_copy(str, key_glyphs[code].desc);
        } else {
            return false;
        }
    }
...
void obs_key_to_str(obs_key_t key, struct dstr *str)
{
    ...
    OSStatus err =
        UCKeyTranslate(plat->layout, code, kUCKeyActionDown,
                       ...
                       LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &dead_key_state, max_length, &len, buffer);
...
static void init_utf_8_strings(void)
{
    ...
    unichar_to_utf8(ctrl_uni, string_control);
    unichar_to_utf8(opt_uni, string_option);
    unichar_to_utf8(shift_uni, string_shift);
    unichar_to_utf8(cmd_uni, string_command);
}

#undef OBS_COCOA_MODIFIER_SIZE
...
void obs_key_combination_to_str(obs_key_combination_t key, struct dstr *str)
{
    ...
    const char *modifier_key     = key_str.len ? key_str.array : "";
...
static void input_method_changed(CFNotificationCenterRef nc __unused, void *observer, CFStringRef name __unused,
                                 const void *object __unused, CFDictionaryRef user_info __unused)
{
