...
function(setup_libobs_target target)
    ...
    foreach(_FIXUP IN LISTS _LIBOBS_FIXUPS)
        ...
        install(DIRECTORY "${_ADDITIONAL_FILES}/${_SOURCE}/"
        ...
        add_custom_command(TARGET ${target} POST_BUILD
            COMMAND "${CMAKE_COMMAND}" -E copy_directory
                "${_ADDITIONAL_FILES}/${_SOURCE}/"
                "${CMAKE_BINARY_DIR}/rundir/$<CONFIG>/${_DESTINATION}"
            VERBATIM)

        if(_SOURCE MATCHES "(libs|exec)(32|64)?")
            install(DIRECTORY "${_ADDITIONAL_FILES}/${_SOURCE}$<IF:$<CONFIG:Debug>,d,r>/"
                DESTINATION "${_DESTINATION}/"
                USE_SOURCE_PERMISSIONS
                PATTERN ".gitignore" EXCLUDE)
...
# Helper function to copy pdb debug symbol files if available (binary/library targets only)
...
function(generate_multiarch_installer)
    if(NOT DEFINED ENV{obsInstallerTempDir})
        message(FATAL_ERROR "Function generate_multiarch_installer requires environment variable 'obsInstallerTempDir' to be set")
    endif()

    add_custom_target(installer_files)
