...
function(set_target_properties_obs target)
  ...
  if(target_type STREQUAL EXECUTABLE)
    ...
    elseif(target STREQUAL inject-helper OR target STREQUAL get-graphics-offsets)
      ...
      cmake_path(RELATIVE_PATH CMAKE_CURRENT_SOURCE_DIR BASE_DIRECTORY "${CMAKE_SOURCE_DIR}" OUTPUT_VARIABLE
                 _project_path)
      ...
      add_custom_command(
        ...
        COMMAND
          ...
          "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.exe"
          "$<$<CONFIG:Debug,RelWithDebInfo>:${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.pdb>"
          "${OBS_OUTPUT_DIR}/$<CONFIG>/${OBS_EXECUTABLE_DESTINATION}"
        COMMENT "Copy get-graphics-offsets (32bit) to data directory"
        VERBATIM)

      install(
        FILES "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.exe"
        DESTINATION "${OBS_EXECUTABLE_DESTINATION}"
        COMPONENT Runtime
        OPTIONAL)
      install(
        FILES "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.pdb"
  ...
  elseif(target_type STREQUAL SHARED_LIBRARY)
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        "$<$<CONFIG:Debug,RelWithDebInfo>:$<TARGET_PDB_FILE:${target}>>"
        "${OBS_OUTPUT_DIR}/$<CONFIG>/${OBS_EXECUTABLE_DESTINATION}/"
      COMMENT "Copy ${target} to library directory"
      VERBATIM)
  ...
  elseif(target_type STREQUAL MODULE_LIBRARY)
    ...
    elseif(target STREQUAL graphics-hook)
      ...
      add_custom_command(
        ...
        COMMAND
          "${CMAKE_COMMAND}" -E copy_if_different
          "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.dll"
          "$<$<CONFIG:Debug,RelWithDebInfo>:${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.pdb>"
          "${OBS_OUTPUT_DIR}/$<CONFIG>/${target_destination}"
        COMMENT "Copy graphics-hook (32bit) to data directory"
        VERBATIM)

      install(
        FILES "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}32.dll"
      ...
      install(
        ...
        COMPONENT Runtime
        OPTIONAL)
    elseif(target STREQUAL obs-virtualcam)
      ...
      add_custom_command(
        ...
        COMMAND
          "${CMAKE_COMMAND}" -E copy_if_different
          "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}-module32.dll"
          "$<$<CONFIG:Debug,RelWithDebInfo>:${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}-module32.pdb>"
          "${OBS_OUTPUT_DIR}/$<CONFIG>/${target_destination}"
        COMMENT "Copy ${target} (32bit) to data directory"
        VERBATIM)

      install(
        FILES "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}-module32.dll"
        DESTINATION "${target_destination}"
        COMPONENT Runtime
        OPTIONAL)
      install(
        FILES "${CMAKE_SOURCE_DIR}/build_x86/${_project_path}/$<CONFIG>/${target}-module32.pdb"
        CONFIGURATIONS RelWithDebInfo Debug
        DESTINATION "${target_destination}"
        COMPONENT Runtime
        OPTIONAL)
    ...
    install(
      ...
      DESTINATION "${target_destination}"
      COMPONENT Runtime
      NAMELINK_COMPONENT Development OPTIONAL)
    ...
    add_custom_command(
      ...
      COMMAND
        ...
        "$<$<CONFIG:Debug,RelWithDebInfo>:$<TARGET_PDB_FILE:${target}>>"
        "${OBS_OUTPUT_DIR}/$<CONFIG>/${target_destination}"
      COMMENT "Copy ${target} to plugin directory"
      VERBATIM)
...
# _bundle_dependencies: Resolve 3rd party dependencies and add them to Windows binary directory
function(_bundle_dependencies target)
  ...
  foreach(config IN ITEMS DEBUG RELWITHDEBINFO RELEASE MINSIZEREL)
    ...
    message(TRACE "Resolved dependencies for config ${config}: ${library_paths_${config}}")
  endforeach()
