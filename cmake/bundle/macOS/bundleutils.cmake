...
message(STATUS "  + OBS - Bundling linked libraries and dependencies")
execute_process(COMMAND /bin/sh -c
    "${_BUNDLER_COMMAND} -a \"${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}\" -cd -of -q -f ${_FIXUP_BUNDLES}")

# Find all dylibs, frameworks and other code elements inside bundle
file(GLOB _OTHER_BINARIES "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/Frameworks/*.app")
...
if(EXISTS "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/PlugIns/obspython.py")
    list(APPEND _OTHER_BINARIES "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/PlugIns/obspython.py")
endif()
...
execute_process(COMMAND /bin/sh -c "cd \"${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/Frameworks\" && ln -fsv libobs.framework/Versions/Current/libobs libobs.0.dylib && ln -fsv libobs.framework/Versions/Current/libobs libobs.dylib")
...
message(STATUS "  + OBS - Codesigning dependencies")
if(EXISTS "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/Frameworks/Sparkle.framework")
    execute_process(COMMAND /bin/sh -c
    ...
    execute_process(COMMAND
        /usr/bin/codesign --force --sign "${_CODESIGN_IDENTITY}" --deep --options runtime "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}/Contents/Frameworks/Sparkle.framework/Versions/A/Resources/Autoupdate.app")
endif()
foreach(_DEPENDENCY IN LISTS  _OTHER_BINARIES _DYLIBS _FRAMEWORKS _OBS_PLUGINS _OBS_SCRIPTING_PLUGINS _QT_PLUGINS)
    if(NOT IS_SYMLINK "${_DEPENDENCY}")
        execute_process(COMMAND
            /usr/bin/codesign --force --sign "${_CODESIGN_IDENTITY}" --options runtime --entitlements "${_CODESIGN_ENTITLEMENTS}" "${_DEPENDENCY}")
    endif()
endforeach()

# Codesign main app
message(STATUS "  + OBS - Codesigning main app")
execute_process(COMMAND /usr/bin/codesign --force --sign "${_CODESIGN_IDENTITY}" --options runtime --entitlements "${_CODESIGN_ENTITLEMENTS}" "${CMAKE_INSTALL_PREFIX}/${_BUNDLENAME}")
