...
if(NOT DEFINED OBS_VERSION_OVERRIDE)
  if(DEFINED RELEASE_CANDIDATE)
    if(RELEASE_CANDIDATE MATCHES "[0-9]+\\.[0-9]+\\.[0-9]+-rc[0-9]+")
      string(REGEX REPLACE "([0-9]+)\\.([0-9]+)\\.([0-9]+)-rc([0-9])+" "\\1;\\2;\\3;\\4"
                           _obs_release_candidate ${RELEASE_CANDIDATE})
      list(GET _obs_release_candidate 0 1 2 _obs_version_canonical)
      set(_obs_version ${RELEASE_CANDIDATE})
    else()
      message(
        FATAL_ERROR
          "Invalid release candidate version supplied - must be <MAJOR>.<MINOR>.<PATCH>-rc<CANDIDATE>."
      )
    endif()
  elseif(DEFINED BETA)
    if(BETA MATCHES "[0-9]+\\.[0-9]+\\.[0-9]+-beta[0-9]+")
      string(REGEX REPLACE "([0-9]+)\\.([0-9]+)\\.([0-9]+)-beta([0-9])+" "\\1;\\2;\\3;\\4"
                           _obs_beta ${BETA})
      list(GET _beta 0 1 2 _obs_version_canonical)
      set(_obs_version ${BETA})
    else()
      message(
        FATAL_ERROR "Invalid beta version supplied - must be <MAJOR>.<MINOR>.<PATCH>-beta<RELEASE>."
      )
    endif()
