...
# target_enable_feature: Adds a feature description for a given target and sets optional compile
# definitions
...
# target_disable_feature: Adds a feature description for a given target and sets optional compile
# definitions
...
macro(find_qt)
  ...
  foreach(component IN LISTS qt_components)
    ...
    if(NOT TARGET Qt::${component} AND TARGET Qt${_QT_VERSION}::${component})
      ...
      set_target_properties(Qt::${component} PROPERTIES INTERFACE_LINK_LIBRARIES
...
function(find_dependencies)
  ...
  message(
    DEBUG
    "[${nested_depth}] Linked interface libraries in target ${var_TARGET}: ${interface_libraries}")
  ...
  foreach(library IN LISTS linked_libraries interface_libraries)
    ...
    elseif(library MATCHES "\\$<.*:[^>]+>")
      ...
      if(library MATCHES "\\$<\\$<PLATFORM_ID:[^>]+>:.+>")
        # Platform-dependent generator expression found - platforms are a comma-separated list of
        # CMake host system identifiers. Convert to CMake list and check if current host os is
        # contained in list.
        string(REGEX REPLACE "\\$<\\$<PLATFORM_ID:([^>]+)>:([^>]+)>" "\\1;\\2" gen_expression
                             "${library}")
      ...
      elseif(library MATCHES "\\$<\\$<BOOL:[^>]+>:.+>")
        # Boolean generator expression found - consider parameter a CMake variable that resolves
        # into a CMake-like boolean value for a simple conditional check.
      ...
      elseif(library MATCHES "\\$<TARGET_NAME_IF_EXISTS:[^>]+>")
        # Target-dependent generator expression found - consider parameter to be a CMake target
        # identifier and check for target existence.
      ...
      elseif(library MATCHES "\\$<.*Qt6::EntryPointPrivate>")
        # Known Qt6-specific generator expression, ignored.
        continue()
      else()
        # Unknown or unimplemented generator expression found - abort script run to either add to
        # ignore list or implement detection.
        message(
          FATAL_ERROR "${library} is an unsupported generator expression for linked libraries.")
      endif()
    endif()
...
function(find_qt_plugins)
  ...
  if(NOT ${library_namespace} MATCHES "Qt[56]?")
    message(
      FATAL_ERROR
        "'find_qt_plugins' has to be called with a valid target from the Qt, Qt5, or Qt6 namespace."
    )
  endif()
...
function(target_export target)
  ...
  message(DEBUG
          "Generating CMake package version configuration file ${target}ConfigVersion.cmake...")
