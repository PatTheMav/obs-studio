...
function(set_target_properties_obs target)
  ...
  if(target_type STREQUAL EXECUTABLE)
    ...
    if(target STREQUAL obs-studio)
      ...
      if(obs_executables)
        foreach(_executable IN LISTS obs_executables)
          add_custom_command(
            TARGET ${target}
            POST_BUILD
            COMMAND "${CMAKE_COMMAND}" -E copy_if_different "$<TARGET_FILE:${_executable}>"
                    "$<TARGET_BUNDLE_CONTENT_DIR:${target}>/MacOS/"
            COMMENT "Copy ${_executable} to application bundle")
        endforeach()
      endif()
      ...
      add_custom_command(
        ...
        COMMENT "Patch Qt styleshets to use larger default font size on macOS")

      install(TARGETS ${target} BUNDLE DESTINATION "." COMPONENT Application)

      _bundle_dependencies(${target})

      if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/cmake/qt.conf")
        target_add_resource(${target} "${CMAKE_CURRENT_SOURCE_DIR}/cmake/qt.conf")
      endif()
      ...
      if(ENABLE_SPARKLE AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/cmake/macos/OBSPublicDSAKey.pem")
        set(OBS_SPARKLE_FEED_URL
            "https://obsproject.com/osx_update/stable/updates_${CMAKE_OSX_ARCHITECTURES}.xml"
            PARENT_SCOPE)
        set(OBS_PUBLIC_KEY_NAME
            OBSPublicDSAKey.pem
            PARENT_SCOPE)
        if(NOT OBS_SPARKLE_KEY)
          set(OBS_SPARKLE_KEY
              ""
              PARENT_SCOPE)
        endif()
        target_add_resource(${target} "${CMAKE_CURRENT_SOURCE_DIR}/cmake/macos/OBSPublicDSAKey.pem")
  ...
  elseif(target_type STREQUAL SHARED_LIBRARY)
    ...
    if(target STREQUAL obs-dal-plugin)
      ...
      add_custom_command(
        ...
        COMMAND
          "${CMAKE_COMMAND}" -E
          $<IF:$<BOOL:$<TARGET_NAME_IF_EXISTS:OBS::studio>>,copy_if_different,true>
          "$<TARGET_BUNDLE_DIR:${target}>" "$<TARGET_BUNDLE_CONTENT_DIR:OBS::studio>/Resources"
  ...
  elseif(target_type STREQUAL MODULE_LIBRARY)
    if(target STREQUAL obspython)
      set_target_properties(
        ${target} PROPERTIES XCODE_ATTRIBUTE_PRODUCT_NAME ${target}
                             XCODE_ATTRIBUTE_PRODUCT_BUNDLE_IDENTIFIER com.obsproject.${target})
      ...
      add_custom_command(
        ...
        COMMAND
          "${CMAKE_COMMAND}" -E
          $<IF:$<BOOL:$<TARGET_NAME_IF_EXISTS:OBS::studio>>,copy_if_different,true>
          "$<TARGET_FILE_DIR:OBS::python>/obspython.py"
          "$<TARGET_BUNDLE_CONTENT_DIR:OBS::studio>/Resources"
    ...
    elseif(target STREQUAL obslua)
      ...
      if(target STREQUAL obs-browser)
        # Good-enough for now as there are no other variants - in _theory_ we should only add the
        # appropriate variant, but that is only known at project generation and not build system
        # configuration
...
# check_info_plist: Macro to check if project ships with Info.plist template
macro(_check_info_plist)
  if(NOT EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/cmake/macos/Info.plist.in")
    message(
      FATAL_ERROR "Target ${target} is missing an Info.plist template in its cmake directory.")
  endif()
endmacro()

# check_entitlements: Macro to check if project ships with entitlements plist
...
# add_entitlements: Macro to add entitlements shipped with project
macro(_add_entitlements)
  if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/cmake/macos/entitlements.plist")
    set_target_properties(
      ${target} PROPERTIES XCODE_ATTRIBUTE_CODE_SIGN_ENTITLEMENTS
...
function(target_install_resources target)
  ...
  if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/data")
    ...
    foreach(data_file IN LISTS data_files)
      cmake_path(RELATIVE_PATH data_file BASE_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/data/"
                 OUTPUT_VARIABLE relative_path)
      cmake_path(GET relative_path PARENT_PATH relative_path)
      target_sources(${target} PRIVATE "${data_file}")
      set_property(SOURCE "${data_file}" PROPERTY MACOSX_PACKAGE_LOCATION
...
# bundle_dependencies: Resolve 3rd party dependencies and add them to macOS app bundle
function(_bundle_dependencies target)
  ...
  foreach(plugin IN LISTS plugins_list)
    ...
    cmake_path(RELATIVE_PATH plugin_path BASE_DIRECTORY "${plugin_stem_dir}" OUTPUT_VARIABLE
               plugin_file_name)
    target_sources(${target} PRIVATE "${plugin}")
    set_source_files_properties(
      "${plugin}" PROPERTIES MACOSX_PACKAGE_LOCATION "plugins/${plugin_file_name}"
