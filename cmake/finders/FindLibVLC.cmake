...
elseif(EXISTS "${LibVLC_INCLUDE_DIR}/libvlc_version.h")
  ...
  string(REGEX REPLACE ".*LIBVLC_VERSION_MAJOR[ \t]+([0-9]+).*" "\\1" VERSION_MAJOR
                       "${_VERSION_STRING}")
  string(REGEX REPLACE ".*LIBVLC_VERSION_MINOR[ \t]+([0-9]+).*" "\\1" VERSION_MINOR
                       "${_VERSION_STRING}")
  string(REGEX REPLACE ".*LIBVLC_VERSION_REVISION[ \t]+([0-9]+).*" "\\1" VERSION_REVISION
                       "${_VERSION_STRING}")
...
find_package_handle_standard_args(
  ...
  VERSION_VAR LibVLC_VERSION REASON_FAILURE_MESSAGE
                             "Ensure that libvlc-dev (vlc on BSD) is installed on the system.")
