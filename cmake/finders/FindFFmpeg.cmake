...
macro(FFmpeg_find_component component)
  ...
  if(FFmpeg_${component}_LIBRARY AND FFmpeg_${component}_INCLUDE_DIR)
    ...
    mark_as_advanced(FFmpeg_${component}_LIBRARY FFmpeg_${component}_INCLUDE_DIR
                     FFmpeg_${component}_IMPLIB)
  endif()
endmacro()
...
macro(FFmpeg_find_dll)
  ...
  string(REGEX REPLACE "([0-9]+)\\.[0-9]+\\.[0-9]+" "\\1" _dll_version
                       "${FFmpeg_${component}_VERSION}")
...
macro(FFmpeg_check_version)
  ...
  elseif(EXISTS "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version.h")
    if(EXISTS "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version_major.h")
      file(STRINGS "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version_major.h"
           _version_string REGEX "^.*VERSION_MAJOR[ \t]+[0-9]+[ \t]*$")
      string(REGEX REPLACE ".*VERSION_MAJOR[ \t]+([0-9]+).*" "\\1" _version_major
                           "${_version_string}")

      file(STRINGS "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version.h"
           _version_string REGEX "^.*VERSION_(MINOR|MICRO)[ \t]+[0-9]+[ \t]*$")
      string(REGEX REPLACE ".*VERSION_MINOR[ \t]+([0-9]+).*" "\\1" _version_minor
                           "${_version_string}")
      string(REGEX REPLACE ".*VERSION_MICRO[ \t]+([0-9]+).*" "\\1" _version_patch
                           "${_version_string}")
    else()
      file(STRINGS "${FFmpeg_${component}_INCLUDE_DIR}/${component_libname}/version.h"
           _version_string REGEX "^.*VERSION_(MAJOR|MINOR|MICRO)[ \t]+[0-9]+[ \t]*$")
      string(REGEX REPLACE ".*VERSION_MAJOR[ \t]+([0-9]+).*" "\\1" _version_major
                           "${_version_string}")
      string(REGEX REPLACE ".*VERSION_MINOR[ \t]+([0-9]+).*" "\\1" _version_minor
                           "${_version_string}")
...
macro(FFmpeg_set_soname)
  if(CMAKE_HOST_SYSTEM_NAME MATCHES "Darwin")
    execute_process(
      COMMAND sh -c
              "otool -D '${FFmpeg_${component}_LIBRARY}' | grep -v '${FFmpeg_${component}_LIBRARY}'"
...
if(EXISTS "${FFmpeg_avutil_INCLUDE_DIR}/libavutil/ffversion.h")
  ...
  string(REGEX REPLACE ".*FFMPEG_VERSION[ \t]+\"n([0-9\\.]+)\".*" "\\1" FFmpeg_VERSION
                       "${_version_string}")
endif()
    ...
    if(IS_ABSOLUTE "${FFmpeg_${component}_LIBRARY}")
      if(DEFINED FFmpeg_${component}_IMPLIB)
        if(FFmpeg_${component}_IMPLIB STREQUAL FFmpeg_${component}_LIBRARY)
          ...
          set_property(TARGET FFmpeg::${component} PROPERTY IMPORTED_IMPLIB
                                                            "${FFmpeg_${component}_IMPLIB}")
        endif()
      else()
      ...
      set_property(TARGET FFmpeg::${component} PROPERTY IMPORTED_LOCATION
                                                        "${FFmpeg_${component}_LIBRARY}")

    else()
      add_library(FFmpeg::${component} INTERFACE IMPORTED)
      set_property(TARGET FFmpeg::${component} PROPERTY IMPORTED_LIBNAME
                                                        "${FFmpeg_${component}_LIBRARY}")
    endif()
