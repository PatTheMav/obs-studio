...
if(CEF_INCLUDE_DIR)
  ...
  string(REGEX REPLACE ".*CEF_VERSION_MAJOR[ \t]+([0-9]+).*" "\\1" VERSION_MAJOR
                       "${_VERSION_STRING}")
  string(REGEX REPLACE ".*CEF_VERSION_MINOR[ \t]+([0-9]+).*" "\\1" VERSION_MINOR
                       "${_VERSION_STRING}")
  string(REGEX REPLACE ".*CEF_VERSION_PATCH[ \t]+([0-9]+).*" "\\1" VERSION_PATCH
                       "${_VERSION_STRING}")
...
if(CMAKE_HOST_SYSTEM_NAME STREQUAL Windows)
  ...
  find_library(
    ...
    PATHS "${CEF_ROOT_DIR}/build/libcef_dll/Release"
          "${CEF_ROOT_DIR}/build/libcef_dll_wrapper/Release" "${CEF_ROOT_DIR}/build/libcef_dll"
...
if(NOT TARGET CEF::Wrapper)
  if(IS_ABSOLUTE "${CEF_LIBRARY_WRAPPER_RELEASE}")
    ...
    set_property(TARGET CEF::Wrapper PROPERTY IMPORTED_LOCATION_RELEASE
                                              "${CEF_LIBRARY_WRAPPER_RELEASE}")
  else()
    add_library(CEF::Wrapper INTERFACE IMPORTED)
    set_property(TARGET CEF::Wrapper PROPERTY IMPORTED_LIBNAME_RELEASE
                                              "${CEF_LIBRARY_WRAPPER_RELEASE}")
  endif()
  set_property(
    TARGET CEF::Wrapper
  ...
  if(CEF_LIBRARY_WRAPPER_DEBUG)
    if(IS_ABSOLUTE "${CEF_LIBRARY_WRAPPER_DEBUG}")
      set_property(TARGET CEF::Wrapper PROPERTY IMPORTED_LOCATION_DEBUG
                                                "${CEF_LIBRARY_WRAPPER_DEBUG}")
    else()
      set_property(TARGET CEF::Wrapper PROPERTY IMPORTED_LIBNAME_DEBUG
                                                "${CEF_LIBRARY_WRAPPER_DEBUG}")
    endif()
    set_property(
      TARGET CEF::Wrapper
  ...
  set_property(TARGET CEF::Wrapper PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                                            "${CEF_INCLUDE_DIR};${CEF_ROOT_DIR}")
endif()
...
if(NOT TARGET CEF::Library)
  ...
  set_target_properties(
    CEF::Library PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${CEF_INCLUDE_DIR};${CEF_ROOT_DIR}"
