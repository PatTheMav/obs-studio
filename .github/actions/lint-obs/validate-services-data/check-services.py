...
CACHE_FILE = f"{CACHE_DIR}/timestamps.json"
...
def set_output(name, value):
    ...
    try:
        ...
        logging.error(f"Writing to github output files failed: {e!r}")
...
async def process_services(session: aiohttp.ClientSession, check_servers: List[str]):
    try:
        ...
        logging.error(f"Could not open services/package file: {e}")
        return 1
    ...
    try:
        ...
        logging.warning(
            f"Could not read cache file, trying to get last artifact (Exception: {e})"
        )
        ...
        try:
            ...
            logging.warning(
                f"Could not fetch cache file, starting fresh. (Exception: {e})"
            )
    ...
    for service in services["services"]:
        ...
        for server, result in zip(service["servers"], zip(*results)):
            ...
            if not is_ok:
                ...
                logging.warning(
                    f"Connecting to server failed: {server['url']} (Reason(s): {failures})"
                )

                if ts := fail_timestamps.get(server["url"], None):
                    if (delta := start_time - ts) >= MINIMUM_PURGE_AGE:
                        logging.info(
                            f'Purging server "{server["url"]}", it has been '
    ...
    if removed_servers:
        ...
        try:
            ...
            logging.error(f"Could not write services/package file: {e}")
            return 1
        else:
            logging.info(
        ...
        try:
            ...
            logging.warning(f"Could not fetch blame for some reason: {e}")
            service_authors = dict()

        # set GitHub outputs
        set_output("failures", len(removed_servers))
...
async def main():
    ...
    if ENV_CI is not None:
        logging.basicConfig(level=logging.INFO, format="%(github_level)s%(message)s")
    else:
        logging.basicConfig(level=logging.INFO, format="%(obs_level)s %(message)s")
    ...
    if CACHE_DIR is None:
        logging.error("Missing environment variable 'CACHE_DIR'.")
        return 1
    ...
    try:
        ...
        logging.error(f"Failed getting required environment variables: {e}")
        return 1
