...
create-archive() {
  ...
  if output="$(compgen -G "obs-studio-*-windows-${BUILD_TARGET}.zip")"; then
    mv "${output}" "${OUTPUT_PATH}/${OUTPUT_NAME}-unsigned.zip"
  fi
...
create-developer-archive() {
  ...
  local OUTPUT_NAME="${OUTPUT_NAME}-plugin-dev.zip"

  7z a "${OUTPUT_PATH}/${OUTPUT_NAME}" "${PWD}"/*
...
package-windows() {
  ...
  : "${OUTPUT_NAME:="obs-studio-windows-${BUILD_TARGET}-${commit_info[hash]}"}"
