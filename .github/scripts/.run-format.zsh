  ...
  case ${1} {
    clang)
      if (( ${+commands[clang-format-13]} )) {
        ...
        if ! is-at-least 13.0.0 ${formatter_version[3]}; then
          log_error "clang-format is not version 13.0 or above (found ${formatter_version[3]}."
          exit 2
        fi

        if ! is-at-least ${formatter_version[3]} 13.0.1; then
          log_error "clang-format is more recent than version 13.0 (found ${formatter_version[3]})."
          exit 2
        fi
      } else {
        log_error "No viable clang-format version found (required 13.0.0)"
        exit 2
      }
      ...
      if (( _loglevel > 1 )) format_args+=(--verbose)
      ;;
    cmake)
      ...
      local -a format_args=()
      if (( _loglevel > 1 )) format_args+=(--log-level debug)
      ;;
    swift)
      ...
      if (( ${+commands[swift-format]} )) {
        ...
        if ! is-at-least 0.50700.0 ${swift_format_version}; then
          log_error "cmake-format is not version 0.50700.0 or above (found ${swift_format_version})."
          exit 2
        fi
      } else {
        log_error "No viable cmake-format version found (required 0.50700.9)"
        exit 2
      }
