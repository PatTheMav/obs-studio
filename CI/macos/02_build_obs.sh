...
build_obs() {
    ...
    if [[ "${PACKAGE}" && "${CODESIGN_IDENT:--}" != '-' ]]; then
        set -o pipefail && xcodebuild ONLY_ACTIVE_ARCH=NO -archivePath "obs-studio.xcarchive" -target obs-studio -destination "generic/platform=macOS,arch=${ARCH}" -parallelizeTargets -hideShellScriptEnvironment archive 2>&1 | xcbeautify
        ...
        set -o pipefail && xcodebuild ONLY_ACTIVE_ARCH=NO -project obs-studio.xcodeproj -target obs-studio -destination "generic/platform=macOS,arch=${ARCH}" -parallelizeTargets -configuration RelWithDebInfo -hideShellScriptEnvironment build 2>&1 | xcbeautify
