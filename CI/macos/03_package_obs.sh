...
notarize_obs() {
    ...
    if [ "${NOTARIZE_IMAGE}" ]; then
        ...
        if [ -f "${OBS_IMAGE}" ]; then
            ...
            error "No notarization app bundle ('OBS.app') or disk image ('${NOTARIZE_IMAGE:-${FILE_NAME}}') found"
            return
        fi
    fi
...
print_usage() {
    echo -e "Usage: ${0}\n" \
            ...
            "--notarize-image [IMAGE]       : Specify existing OBS image for notarization\n" \
            "--notarize-bundle [BUNDLE]     : Specify existing OBS bundle for notarization\n" \
