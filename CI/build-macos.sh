...
#   -b, --bundle                   : Create relocatable app bundle
#                                    (default: off)
...
print_usage() {
    ...
    echo -e "Usage: ${0}\n" \
            ...
            "-v, --verbose                  : Enable more verbose build process output\n" \
            "-d, --skip-dependency-checks   : Skip dependency checks (default: off)\n" \
            "-b, --bundle                   : Create relocatable app bundle (default: off)\n" \
...
print_deprecation() {
    ...
    if [ "${1}" = "-s" ]; then
        echo -e "The macOS build script system has changed:\n" \
                ...
                " - To bundle OBS into a relocatable app bundle, run the script 'CI/macos/02_build_obs.sh --bundle\n" \
