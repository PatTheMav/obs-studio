...
std::pair<OBS::TimePoint, std::string> buildCrashLogUploadContent(std::string crashLogFileContent)
{
...
bool CrashHandler::didOBSCrashBefore()
{
	return isSentinelEnabled && isCrashDetected_;
}
...
void CrashHandler::uploadLastCrashLog()
{
	...
	if (lastLocalCrashLogFile.empty()) {
		return;
	}

	bool newCrashDetected = lastLocalCrashLogFile != lastCrashLogFile_;

	if (newCrashDetected || lastCrashLogURL_.empty()) {
...
void CrashHandler::updateCrashLogFromConfig()
{
	...
	int lastCrashLogUploadTimestamp = config_get_int(appConfig, "CrashHandler", "last_crash_log_time");
...
void CrashHandler::uploadCrashLogToServer()
{
	...
	auto crashLogUploadContent = buildCrashLogUploadContent(crashLogFileContent);
...
void CrashHandler::handleExistingCrashLogUpload()
{
	...
	if (config_has_user_value(appConfig, "CrashHandler", "last_crash_log_url")) {
		QString crashLogUrlString = QT_UTF8(config_get_string(appConfig, "CrashHandler", "last_crash_log_url"));
...
void CrashHandler::crashLogUploadResultHandler(const QString &uploadResult, const QString &error)
{
	...
	try {
		...
		config_set_string(appConfig, "CrashHandler", "last_crash_log_file",
				  lastCrashLogFile_.u8string().c_str());
		config_set_int(appConfig, "CrashHandler", "last_crash_log_time", uploadTimePoint_c);
		config_set_string(appConfig, "CrashHandler", "last_crash_log_url", crashLogUrl.c_str());
		config_save_safe(appConfig, "tmp", nullptr);

		QString crashLogFile = QString::fromStdString(lastCrashLogFile_.u8string());
		QString crashLogUrlString = QString::fromStdString(crashLogUrl);
