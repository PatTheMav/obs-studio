...
// MARK: - Remaining OBSBasic implementation
...
static void SaveAudioDevice(const char *name, int channel, obs_data_t *parent, std::vector<OBSSource> &audioSources)
{
...
void OBSBasic::ClearSceneData()
{
	...
	if (!orphan_sources.empty()) {
		...
		if (!clearingFailed) {
			...
			std::string orphan_names =
				std::accumulate(orphan_sources.begin(), orphan_sources.end(), std::string(""),
...
void OBSBasic::Load(const char *file, bool remigrate)
{
	...
	if (!data) {
		...
		const std::string name = path.stem().u8string();
