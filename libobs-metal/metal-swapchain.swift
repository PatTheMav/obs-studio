...
public func device_load_swapchain(device: UnsafeRawPointer, swap: UnsafeRawPointer) {
    ...
    if device.swapChains == nil {
        device.swapChains = [OBSSwapChain]()
    }
...
///
/// The ownership of the shared pointer is transferred into this function and thus the instance is placed under Swift's
/// memory management again.
@_cdecl("gs_swapchain_destroy")
public func gs_swapchain_destroy(swapChain: UnsafeMutableRawPointer) {
    let _ = retained(swapChain) as OBSSwapChain
}
