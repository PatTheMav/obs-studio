...
void init_window(cocoa_window_t cw, obs_data_t *settings)
{
	...
	for (NSDictionary *dict in enumerate_windows()) {
		...
		if (owner_names_match && (ids_match || window_names_match)) {
			...
			NSNumber *_window_id = (NSNumber *)dict[WINDOW_NUMBER];
			cw->window_id = _window_id.intValue;
			NSNumber *_owner_pid = (NSNumber *)dict[OWNER_PID];
			cw->owner_pid = _owner_pid.intValue;
