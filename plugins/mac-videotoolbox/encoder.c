				       ...
				       enum video_colorspace cs)
{
	...
	if (cs == VIDEO_CS_2100_PQ) {
		...
		values[3] = obs_to_vt_masteringdisplay(hdr_nominal_peak_level);
		values[4] =
			obs_to_vt_contentlightlevelinfo(hdr_nominal_peak_level);

	} else if (cs == VIDEO_CS_2100_HLG) {
		values[3] = obs_to_vt_masteringdisplay(1000);
		values[4] = obs_to_vt_contentlightlevelinfo(1000);
	}
...
static obs_properties_t *vt_properties_h26x(void *data __attribute__((unused)),
					    void *type_data)
{
